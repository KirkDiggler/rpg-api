...
/*
// convertExternalEquipmentToEquipmentData converts external EquipmentData to internal EquipmentData
func convertExternalEquipmentToEquipmentData(equipment *external.EquipmentData) *dnd5e.EquipmentData {
	if equipment == nil {
		return nil
	}
	...
	// Cost will be stored in GearData when needed

	// Convert weight from float32 to int32 (tenths of pounds)
	equipmentData.Weight = int32(equipment.Weight * 10)

	// Convert weapon-specific data
	if equipment.WeaponCategory != "" || equipment.Damage != nil {
		weaponData := &dnd5e.WeaponData{
			WeaponCategory: equipment.WeaponCategory,
			Properties:     equipment.Properties,
			Range:          equipment.WeaponRange,
		}

		if equipment.Damage != nil {
			weaponData.DamageDice = equipment.Damage.DamageDice
			weaponData.DamageType = equipment.Damage.DamageType
		}

		// Parse range values from properties or other fields
		// TODO(#45): Extract range values from API data

		equipmentData.WeaponData = weaponData
	}

	// Convert armor-specific data
	if equipment.ArmorCategory != "" || equipment.ArmorClass != nil {
		armorData := &dnd5e.ArmorData{
			ArmorCategory:       equipment.ArmorCategory,
			StrMinimum:          int32(equipment.StrengthMinimum),
			StealthDisadvantage: equipment.StealthDisadvantage,
		}

		if equipment.ArmorClass != nil {
			armorData.BaseAC = int32(equipment.ArmorClass.Base)
			armorData.DexBonus = equipment.ArmorClass.DexBonus
			// Set HasDexLimit based on armor category
			if equipment.ArmorCategory == "medium" {
				armorData.HasDexLimit = true
				armorData.MaxDexBonus = 2 // Standard for medium armor
			} else if equipment.ArmorCategory == "heavy" {
				armorData.HasDexLimit = true
				armorData.MaxDexBonus = 0 // No dex bonus for heavy armor
			}
		}

		equipmentData.ArmorData = armorData
	}

	// Handle generic gear
	if equipment.EquipmentType == "gear" || (equipment.WeaponCategory == "" && equipment.ArmorCategory == "") {
		equipmentData.GearData = &dnd5e.GearData{
			GearCategory: equipment.Category,
			Properties:   equipment.Properties,
		}
		// Some gear is stackable (like arrows, rations, etc.)
		// TODO(#45): Determine stackability based on item type
	}
...
/*
// getEquipmentData fetches equipment data from external client
// TODO(#134): This will be used when equipment storage is implemented
func (o *Orchestrator) getEquipmentData(ctx context.Context, itemID string) (*dnd5e.EquipmentData, error) {
	// Try to get equipment data from external client
	equipmentData, err := o.externalClient.GetEquipmentData(ctx, itemID)
	if err != nil {
		return nil, err
	}
	if equipmentData == nil {
		return nil, errors.NotFound("equipment data not found")
	}

	// Convert external equipment to our internal EquipmentData
	return o.convertExternalEquipmentToEquipmentData(equipmentData), nil
}
*/

/*
// convertExternalEquipmentToEquipmentData converts external equipment to our internal EquipmentData format
func (o *Orchestrator) convertExternalEquipmentToEquipmentData(equipment *external.EquipmentData) *dnd5e.EquipmentData {
	if equipment == nil {
		return nil
	}

	// Convert weight from float to int32 (tenths of pounds)
	weight := int32(equipment.Weight * 10)

	data := &dnd5e.EquipmentData{
		ID:         equipment.ID,
		Name:       equipment.Name,
		Type:       equipment.EquipmentType,
		Category:   equipment.Category,
		Weight:     weight,
		Properties: equipment.Properties,
		Stackable:  false, // TODO: Determine from equipment type
	}

	// Convert weapon data if present
	if equipment.WeaponCategory != "" && equipment.Damage != nil {
		data.WeaponData = &dnd5e.WeaponData{
			DamageDice:     equipment.Damage.DamageDice,
			DamageType:     equipment.Damage.DamageType,
			WeaponCategory: strings.ToLower(equipment.WeaponCategory),
			Range:          equipment.WeaponRange,
			Properties:     equipment.Properties,
		}
		// Set range values based on weapon range type
		if equipment.WeaponRange == "Ranged" {
			// TODO: Parse actual range values from equipment data
			data.WeaponData.NormalRange = 30
			data.WeaponData.LongRange = 120
		}
	}

	// Convert armor data if present
	if equipment.ArmorCategory != "" && equipment.ArmorClass != nil {
		data.ArmorData = &dnd5e.ArmorData{
			ArmorCategory:       strings.ToLower(equipment.ArmorCategory),
			BaseAC:              int32(equipment.ArmorClass.Base),
			DexBonus:            equipment.ArmorClass.DexBonus,
			HasDexLimit:         false, // TODO: Parse from armor category
			MaxDexBonus:         0,
			StrMinimum:          int32(equipment.StrengthMinimum),
			StealthDisadvantage: equipment.StealthDisadvantage,
		}
		// Set dex limits based on armor category
		if equipment.ArmorCategory == "Medium" {
			data.ArmorData.HasDexLimit = true
			data.ArmorData.MaxDexBonus = 2
		} else if equipment.ArmorCategory == "Heavy" {
			data.ArmorData.HasDexLimit = true
			data.ArmorData.MaxDexBonus = 0
		}
	}

	// Create basic gear data with cost info
	if equipment.Cost != nil {
		// Parse cost to copper pieces (1 gp = 100 cp)
		costInCopper := int32(equipment.Cost.Quantity * 100)
		if equipment.Cost.Unit == "sp" {
			costInCopper = int32(equipment.Cost.Quantity * 10)
		} else if equipment.Cost.Unit == "cp" {
			costInCopper = int32(equipment.Cost.Quantity)
		}

		data.GearData = &dnd5e.GearData{
			CostInCopper:       costInCopper,
			RequiresAttunement: false, // TODO: Parse from properties
		}
	}
...
// TODO(#134): The following helper methods will be used when equipment storage is implemented

/*
// getCharacterProficiencies retrieves all equipment proficiencies for a character
func (o *Orchestrator) getCharacterProficiencies(ctx context.Context, character *dnd5e.Character) ([]string, error) {
	proficiencies := []string{}

	// Get class proficiencies
	if character.ClassID != "" {
		classData, err := o.externalClient.GetClassData(ctx, character.ClassID)
		if err == nil && classData != nil {
			// Add armor proficiencies
			proficiencies = append(proficiencies, classData.ArmorProficiencies...)
			// Add weapon proficiencies
			proficiencies = append(proficiencies, classData.WeaponProficiencies...)
		}
	}

	// Get race proficiencies
	if character.RaceID != "" {
		raceData, err := o.externalClient.GetRaceData(ctx, character.RaceID)
		if err == nil && raceData != nil {
			// Add any equipment-related proficiencies from race
			for _, prof := range raceData.Proficiencies {
				if strings.Contains(strings.ToLower(prof), "armor") ||
				   strings.Contains(strings.ToLower(prof), "weapon") ||
				   strings.Contains(strings.ToLower(prof), "shield") {
	...
	// TODO: Add background proficiencies, feat proficiencies, etc.

	return proficiencies, nil
}

// populateEquippedItems populates the equipped items map for validation
func (o *Orchestrator) populateEquippedItems(
	ctx context.Context,
	slots *dnd5e.EquipmentSlots,
	...
	addItem := func(slot string, item *dnd5e.InventoryItem) {
		if item != nil {
			equipData, err := o.getEquipmentData(ctx, item.ItemID)
			if err == nil {
				equippedItems[slot] = NewItemAdapter(item, equipData)
			}
		}
	}

	// Add all equipped items
	addItem(dnd5e.EquipmentSlotMainHand, slots.MainHand)
	addItem(dnd5e.EquipmentSlotOffHand, slots.OffHand)
	addItem(dnd5e.EquipmentSlotArmor, slots.Armor)
	addItem(dnd5e.EquipmentSlotHelmet, slots.Helmet)
	addItem(dnd5e.EquipmentSlotBoots, slots.Boots)
	addItem(dnd5e.EquipmentSlotGloves, slots.Gloves)
	addItem(dnd5e.EquipmentSlotCloak, slots.Cloak)
	addItem(dnd5e.EquipmentSlotAmulet, slots.Amulet)
	addItem(dnd5e.EquipmentSlotRing1, slots.Ring1)
	addItem(dnd5e.EquipmentSlotRing2, slots.Ring2)
	addItem(dnd5e.EquipmentSlotBelt, slots.Belt)
}

// getAttunedItems returns all attuned items for a character
func (o *Orchestrator) getAttunedItems(character *dnd5e.Character) []items.Item {
	attunedItems := []items.Item{}

	// Check inventory for attuned items
	if character.Inventory != nil {
		for _, item := range character.Inventory {
			if item.IsAttuned {
				// Note: We would need equipment data to create proper item adapter
				// For now, this is a placeholder
				copiedItem := item
				attunedItems = append(attunedItems, &ItemAdapter{inventoryItem: &copiedItem})
			}
		}
	}

	// Check equipped items for attunement
	if character.EquipmentSlots != nil {
		checkSlot := func(item *dnd5e.InventoryItem) {
			if item != nil && item.IsAttuned {
				attunedItems = append(attunedItems, &ItemAdapter{inventoryItem: item})
			}
		}

		checkSlot(character.EquipmentSlots.MainHand)
		checkSlot(character.EquipmentSlots.OffHand)
		checkSlot(character.EquipmentSlots.Armor)
		checkSlot(character.EquipmentSlots.Helmet)
		checkSlot(character.EquipmentSlots.Boots)
		checkSlot(character.EquipmentSlots.Gloves)
		checkSlot(character.EquipmentSlots.Cloak)
		checkSlot(character.EquipmentSlots.Amulet)
		checkSlot(character.EquipmentSlots.Ring1)
		checkSlot(character.EquipmentSlots.Ring2)
		checkSlot(character.EquipmentSlots.Belt)
	}

	return attunedItems
}

// mapEquipmentError maps rpg-toolkit validation errors to our error types
func (o *Orchestrator) mapEquipmentError(err error) error {
	if err == nil {
		return nil
	}

	// Map specific error types from rpg-toolkit to our errors
	switch err.Error() {
	case "character lacks required strength":
		return errors.InvalidArgument("character does not meet strength requirement for this armor")
	case "character lacks required proficiency":
		return errors.InvalidArgument("character is not proficient with this equipment")
	case "slot already occupied":
		return errors.InvalidArgument("equipment slot is already occupied")
	case "incompatible slot":
		return errors.InvalidArgument("item cannot be equipped in this slot")
	case "attunement limit reached":
		return errors.InvalidArgument("character has reached maximum attunement limit")
	case "two-handed weapon conflict":
		return errors.InvalidArgument("cannot equip two-handed weapon while holding other items")
	default:
		return errors.InvalidArgument(err.Error())
	}
}

// updateCharacterEncumbrance calculates and updates the character's encumbrance
func (o *Orchestrator) updateCharacterEncumbrance(ctx context.Context, character *dnd5e.Character) {
	if character == nil {
		return
	}

	// Initialize encumbrance if not present
	if character.Encumbrance == nil {
		character.Encumbrance = &dnd5e.EncumbranceInfo{}
	}
	...
	if character.Inventory != nil {
		for _, item := range character.Inventory {
			equipData, err := o.getEquipmentData(ctx, item.ItemID)
			if err == nil && equipData != nil {
				totalWeight += float64(equipData.Weight) * float64(item.Quantity) / 10.0
			}
		}
	}
	...
	if character.EquipmentSlots != nil {
		addWeight := func(item *dnd5e.InventoryItem) {
			if item != nil {
				equipData, err := o.getEquipmentData(ctx, item.ItemID)
				if err == nil && equipData != nil {
					totalWeight += float64(equipData.Weight) * float64(item.Quantity) / 10.0
		...
		addWeight(character.EquipmentSlots.MainHand)
		addWeight(character.EquipmentSlots.OffHand)
		addWeight(character.EquipmentSlots.Armor)
		addWeight(character.EquipmentSlots.Helmet)
		addWeight(character.EquipmentSlots.Boots)
		addWeight(character.EquipmentSlots.Gloves)
		addWeight(character.EquipmentSlots.Cloak)
		addWeight(character.EquipmentSlots.Amulet)
		addWeight(character.EquipmentSlots.Ring1)
		addWeight(character.EquipmentSlots.Ring2)
		addWeight(character.EquipmentSlots.Belt)
	}

	// Update encumbrance info (convert pounds to tenths of pounds)
	character.Encumbrance.CurrentWeight = int32(totalWeight * 10)

	// Calculate carrying capacity (STR * 15 for D&D 5e, in tenths of pounds)
	strength := int(character.AbilityScores.Strength)
	if strength == 0 {
		strength = 10 // Default
	}
	carryingCapacity := int32(strength * 15 * 10) // Convert to tenths
	character.Encumbrance.CarryingCapacity = carryingCapacity
	character.Encumbrance.MaxCapacity = carryingCapacity * 2 // Push/drag/lift limit

	// Determine encumbrance level
	currentWeight := int32(totalWeight * 10)
	if currentWeight > carryingCapacity {
		character.Encumbrance.Level = "heavily_encumbered"
	} else if currentWeight > carryingCapacity*2/3 {
		character.Encumbrance.Level = "encumbered"
	} else {
		character.Encumbrance.Level = "normal"
	}
}
*/
