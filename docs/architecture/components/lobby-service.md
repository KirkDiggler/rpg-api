...
- **Player spawn positions are still a line placeholder, now anchored at the dungeon
  ...
  crypt key, or a named default — to a literal `tkenc.DungeonParams`, and
  `enc.InitDungeon(...)` runs before the member loop. The crypt spec is 3 regions
  (entrance → corridor → boss) + 2 plain doors + a designated entrance cell +
  per-region archetype tags + an opaque `Theme`, all toolkit-generated by key —
