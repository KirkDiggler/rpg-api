...
- **Player spawn positions are still a line placeholder, now anchored at the dungeon
  ...
  dungeon is now TWO chambers, not one room** (rpg-api#676, The Dungeon wave 2 Slice 2:
  `enc.InitTwoChamberRoom(...)` runs before the member loop — 2 chambers + 1 plain door
  + a designated entrance cell + per-chamber region tags, all toolkit-generated by key;
  `roomCenterHex()` is gone). 4 goblins (2 per chamber) are seeded at verified-unseen
  positions, region-tag-scoped — see `docs/status.md`'s "The Dungeon wave 2 Slice 2"
  entry. Only the PLAYER placement within chamber 1 is still the line placeholder; real
  player spawn-point selection is future work.
