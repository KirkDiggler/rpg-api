...
- **Player spawn positions are still a line placeholder, now anchored at the dungeon
  ...
  are gone from this file). 4 goblins (2 in the entrance region, 2 in the terminal/boss
  region — the dungeon's two occupied ENDPOINTS by chain position, not every region) are
  seeded at verified-unseen positions, region-archetype-scoped — see `docs/status.md`'s
  "N-region dungeon by key" entry. Only the PLAYER placement within the entrance region
  is still the line placeholder; real player spawn-point selection is future work.
  Deciding how many/which monsters populate an arbitrary region by its archetype (the
  interior corridor region gets none today) is rpg-api#689, out of scope here.
- **Out-of-sight goblin placement can now fail against the toolkit's smaller crypt
  dimensions (rpg-api#696, filed alongside #694)** — up to ~33% of `StartEncounter`
  calls for a full 4-player party, on the real entropy-seeded path every production
  caller uses. See `docs/status.md`'s "Known rough edges" entry for full measurements;
  not fixed by #694 (out of its scope — obstacle consumption only).
