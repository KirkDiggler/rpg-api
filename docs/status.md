...
**Released provider pin — CLEAN (toolkit #876, 2026-08-03)** — API now consumes the published `github.com/KirkDiggler/rpg-toolkit/encounter v0.47.0`, the `encounter/v0.47.0` tag on merge commit `7c7a587c9f786193a71602db488184846c0de54e`. The temporary `github.com/KirkDiggler/rpg-toolkit/encounter => ./local-toolkit/encounter` loop is off: `go.mod` has no `replace`, `local-toolkit/` is absent, and the release gate verifies this graph with `GOWORK=off`. The provider's **Auto Tag Modules (Safe)** run `30839494671` reached terminal `success` before this pin.
