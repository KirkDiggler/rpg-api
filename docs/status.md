...
**Released encounter pin — CLEAN (toolkit encounter v0.52.0, 2026-08-09)** — PR #782 consumes published `github.com/KirkDiggler/rpg-toolkit/encounter v0.52.0`, carrying the native Dungeon YAML v0.4 Wave A compile/runtime/persistence provider from toolkit PR #900. The module sum is `h1:LYi/RNcxxGl4kbtZKQ8jUHcK1BKTy/FLBX4Afs7cDE8=`. `go.mod` has no `replace`, `go.work` is absent, and the integration uses the immutable release only.
