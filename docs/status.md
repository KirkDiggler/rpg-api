...
**Dungeon YAML v0.4 placement offset — LOCAL INTEGRATION, PROVIDER RELEASES PENDING (rpg-api#783, 2026-08-09)** — `PutDungeon` projects toolkit-owned placement source path/ref/absolute cell/applicable facing/effective blockers and optional world-axis offset without interpreting any value. Encounter creation persists the same optional triple through toolkit `ObstacleData` / `MonsterData`; reconnect and live `HexRecord.contents` projection preserve omission versus explicit zero and signed XYZ while fog continues to suppress unauthorized placements. Movement changes only the canonical origin, total empty records vacate the offset, and later authored-source edits do not recompute existing encounter snapshots. Local integration is green against temporary toolkit commit `25ed2ed` and proto PR #220 head `74298bc`; neither override/version is committable. This branch is not merge-ready until T/P merge, publish real versions, and `go.mod` pins those releases with no `replace`.
