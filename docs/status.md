...
**Semantic regions/fog consumer (rpg-api#773, API PR in progress on `feat/773-region-fog-projections`; not merged/deployed, 2026-08-06)** — `PutDungeon` remains a thin field-for-field floor-plan consumer and now projects provider-authored regions; encounter snapshots consume `AuthorizedZones`, while Wave 1 live translations consume provider-published observations directly. The branch pins released `rpg-toolkit/encounter v0.50.0`. This is API implementation evidence only: no API merge, deployment, or live verification is claimed. Direct `os.WriteFile` durability and concurrent transaction durability remain separately tracked by rpg-api#772.
...
**Released semantic-region provider pin — CLEAN (toolkit encounter v0.50.0, 2026-08-06)** — This branch consumes published `github.com/KirkDiggler/rpg-toolkit/encounter v0.50.0`, tag/origin hash `3cee912cf87ce4dc319bdaeb86cdd2943b80fc36`, module sum `h1:69Z0p6hw+tdId5t8G3jGDEbLobwpk684jYSPM70e6UM=`, after successful Auto Tag run `31131606077`. The temporary local override is off: `go.mod` has no `replace`, `local-toolkit/` is absent, and the release gate verifies the graph with `GOWORK=off`.
