...
**Dungeon YAML v0.4 Wave A API checkpoint (rpg-api#780, provider-blocked, 2026-08-09)** — pinned released generated Go proto `v0.0.0-20260809002602-f4d6396df528`; added resolved floor-source and optional-entrance handler mapping, exact validation-path transport, and a protobuf-free `Compiler` seam with explicit draft versus strict inputs. Mock-driven ring/tiny/disconnected tests and a production authored-source/running-snapshot isolation regression are green. Finalization is blocked on rpg-toolkit#897 publishing the native region-union/draft/strict provider; no topology logic or local toolkit replacement is present. API PR #781 remains open and must be incorporated from `dev` rather than duplicated here. See `docs/architecture/components/dungeon-wave-a.md`.
