...
**Guild-scoped immutable composition repository provider (rpg-api#921,
2026-09-05)** — typed composition definitions and immutable source revisions now have a
focused Redis repository with generated IDs, timestamps, one guild-scoped cluster hash
tag, no TTL, exact pinned reads, deterministic current-head listing, and Lua-atomic
create/expected-head append preflights. Corrupt stored identity/source metadata fails
closed; `ListDefinitions` fails the whole read rather than silently omitting an invalid
indexed record. Source version 1 validation is part of the immutable-read contract and
must not be retroactively tightened when later versions are added.

This is repository-only infrastructure: it is not registered in server DI or exposed by
RPC, and it does not implement guild authorization, dungeon/world placement, toolkit
rules, asset generation, or rendering. Lua validation/conflict refusal is atomic, but
end-to-end request acknowledgment is not exactly-once: a go-redis retry after a lost
reply can observe the first committed attempt as a generated-ID collision or stale
head. Before RPC exposure, the service integration must add a concrete acceptance test
for idempotent request handling or committed-outcome reconciliation.
