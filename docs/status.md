...
**Local provider loop status — ACTIVE (intentional, uncommitted)** — exactly one replace directive is active: `github.com/KirkDiggler/rpg-toolkit/encounter => ./local-toolkit/encounter`, synced from toolkit PR #876 head `2adb0c62ad7f07b3cb2b74e565d81c5f38b35d0b` with `scripts/toolkit-local-override.sh on --src /home/kirk/game-dev/rpg-toolkit`. `local-toolkit/` is gitignored and this `go.mod` replace must remain uncommitted until the provider PR publishes a real module version.
