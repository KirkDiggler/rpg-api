...
**World composition repository provider (rpg-api#921, 2026-09-06)** —
`internal/repositories/composition` stores toolkit `composition.Data` directly. Create,
Get, and List use one Redis hash per world (composition ID fields with JSON values), no
TTL, create-without-overwrite semantics, and deterministic ID ordering. Focused
miniredis tests cover round trips, world isolation, duplicates, missing/empty reads,
storage/decode failures, and snapshot independence.

This pass is repository-only: it is not registered in server DI or exposed by RPC. The
separate edge integration must resolve the authenticated world and translate the merged
Create/Get/List proto contract; the repository does not implement authorization,
toolkit rules, asset generation, or rendering.
