...
`SessionManager` interface (`StartSession`, `Spawn`, `Join`, `PlaceNPC`,
`Status`, `End`) the orchestrator consumes, with a `go:generate` mock at
`internal/orchestrators/lobby/mock/mock_session_manager.go`; the real `*sdk.Manager`
still satisfies it structurally, so production wiring is unchanged (no adapter,
no second manager, no provider-pin change). `LobbySuite` and the new
`StartContractSuite` run on that mock, a registry mock, the in-memory lobby
repository and the broker — no miniredis, no shipped YAML, no session
orchestrator, no playable character — and pin exact launch inputs and the
required order: `StartSession` → every `Spawn` → every `Join` → default-key
`PlaceNPC` → `Save` → `Publish`, plus refusal/partial-failure gates asserting no
SDK call, no final write and no `EncounterStarted` event. This is a bounded
pilot, not finished migration: `SessionStackSuite` and the lobby handler suite
keep their real miniredis-backed `session.Manager` and remain active, so not
every lobby/API test is isolated. Four real-stack gate cases were removed only
after each mapped to a `StartContractSuite` case; coverage disposition:
...
The four removed cases map as `NotHost`→`NonHost_Refused`,
`NotAllReady`→`NotReadyMember_Refused`,
...
composition methods: under Discord auth it strictly validates one canonical non-zero
uint64 `x-rpg-guild-id`, verifies the authenticated user through Discord's same-token
current-member endpoint, and installs that GuildID directly as trusted WorldID. Under
actual Dev auth it uses `RPG_DEV_WORLD_ID` (default `test-world`) only when
`AUTH_DEV_MODE=true`; Discord auth on that same server still verifies membership.
Private credentials are removed before handlers, and global/stream RPC scope is
unchanged.

Successful membership decisions are cached in process for at most 30 seconds and 1,024
entries by SHA-256 token digest plus GuildID. Denials and stale results are never cached.
A membership `401` evicts the existing identity entry and all membership entries for
that token; broader identity-cache raw-key/bounds hardening remains deferred to
rpg-api#937, and OAuth transaction binding remains deferred to rpg-project#403.

Every handler method requires body WorldID to equal trusted context before service or
repository access and supplies only the trusted value downstream. Create/Get/List reject
nil, wrong-world, or wrong-ID service output as an integrity failure. Create and Delete
retain `RPG_AUTHORING_ENABLED=1`; Get and List require membership only. No owner, admin,
role, creator ACL, world registry, travel, or global RPC migration was added.

The orchestrator still mints IDs and persists toolkit `composition.Data` directly. The
repository retains one Redis hash per world, create-without-overwrite, deterministic
listing, stored-envelope WorldID/ID validation, and idempotent delete. Focused tests
cover hostile selector metadata, same-token identity-cache hits, provider status
mapping, cache expiry/bounds/races and `401` invalidation, trusted body/context checks,
wrong service output, production/Dev registration, authoring gates, and handler-through-
miniredis snapshots.
...
thin `StartSession → Spawn → Join → Save → Publish` consumer (the reference tomb
adds one demo `PlaceNPC` after the joins): its complete runtime-character /
