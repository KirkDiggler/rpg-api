...
`CompositionService` exposes the published proto300 Create/Get/List JSON contract only
...
existing `RPG_AUTHORING_ENABLED=1` mutation gate; Get and List remain available when
that mutation flag is off.
...
ID fields with serialized Data values), no TTL, create-without-overwrite semantics, and
deterministic ID ordering. Focused tests cover request/auth/world/JSON validation,
ordinary API-error to gRPC mapping, dev/non-dev registration, and full handler through
miniredis Create/Get/List with independent immutable snapshots. This is deliberately a
