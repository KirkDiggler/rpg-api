...
**Dungeon Builder Wave 0 canvas-provider consumer (rpg-api#770, PR #771 OPEN on `feat/770-v03-canvas-put-dungeon`; not merged/deployed, 2026-08-05)** — `PutDungeon` is a thin field-for-field consumer of the toolkit floor-plan provider: it preserves the opaque prior compilation/no-overrides path and projects canvas cells, authored edges, and optional door IDs. The branch uses generated proto root `v0.1.120` artifact `ba7eda1` (`gen/go v0.0.0-20260805195341-ba7eda1f1833`) and `rpg-toolkit/encounter v0.49.1`. Wave 0 is implementation/independent-delta-review pending; its acceptance boundary is validation-failure nonmutation. Direct `os.WriteFile` durability and concurrent transaction durability are not claimed here; rpg-api#772 tracks them.
...
**Released Wave 0 provider pin — CLEAN (toolkit #883/#886, 2026-08-05)** — This branch consumes the published `github.com/KirkDiggler/rpg-toolkit/encounter v0.49.1`, the `encounter/v0.49.1` tag on merge commit `49962520198fc508dde956b38c7773041a807e66`. The temporary `github.com/KirkDiggler/rpg-toolkit/encounter => ./local-toolkit/encounter` loop is off: `go.mod` has no `replace`, `local-toolkit/` is absent, and the release gate verifies this graph with `GOWORK=off`. The provider's **Auto Tag Modules (Safe)** run `31072886227` reached terminal `success` before this pin.
