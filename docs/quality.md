...
Cleric/Bless consumer (#975/#978): boundary coverage now selects a returned Life Domain, resumes saved spell choices, finalizes natively, enters combat from world mode, and casts Bless and both heals with spending/live/reloaded Story assertions. Both stale-target policies use that native character. Owner-private data now passes its native regression with root v0.165.1; Knowledge Domain extra choices now use released provider input support; the Knowledge round-trip integration covers both skill/language requirements and the returned shared proficiency ranks. No broader grade increase or UI/deployment claim; see the [handoff](how-to/cleric-bless-web-handoff.md).
