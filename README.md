...
`sandboxseed` is dev-only. The default fixture preserves the existing toolkit sandbox characters. The repeatable Weapon Gallery owns 30 canonical weapon rows in provider order: Scimitar has quantity 2, and every other weapon has quantity 1. Re-seeding preserves its stable character ID and non-weapon state. For the rpg-dev Docker stack, use the wrapper so the gallery fixture targets the Redis container instead of guessing `localhost`:
